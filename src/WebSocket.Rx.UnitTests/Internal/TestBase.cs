...
namespace WebSocket.Rx.UnitTests.Internal;
...
public abstract class TestBase(ITestOutputHelper output)
{
        ...
        int? timeoutMs = null)
    {
        ...
        await using var registration = cts.Token.Register(() =>
        {
            ...
            try { Output.WriteLine($"[TIMEOUT] {msg}"); } catch { }
        ...
        int? timeoutMs = null)
    {
        ...
        {
            try { if (condition()) tcs.TrySetResult(true); } catch (Exception ex) { tcs.TrySetException(ex); }
        });
