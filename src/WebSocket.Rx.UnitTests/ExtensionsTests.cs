...
public class ExtensionsTests
{
    ...
    public async Task ClientSendInstant_ShouldUseSendInstantAsync()
    {
        ...
        {
            SendInstantTextHandler = (_, _) => Task.FromResult(true),
            SendInstantBinaryHandler = (_, _) => Task.FromResult(false)
        };
        ...
        Assert.Single(client.BinarySendInstantCalls);
        Assert.Empty(client.TextSendCalls);
        Assert.Empty(client.BinarySendCalls);
    ...
    public async Task ServerSendInstant_ShouldUseSendInstantAsync()
    {
        ...
        {
            SendInstantTextHandler = (_, _, _) => Task.FromResult(true),
            SendInstantBinaryHandler = (_, _, _) => Task.FromResult(false)
        };
        ...
        Assert.Single(server.BinarySendInstantCalls);
        Assert.Empty(server.TextSendCalls);
        Assert.Empty(server.BinarySendCalls);
    ...
    private sealed class TestReactiveWebSocketClient : IReactiveWebSocketClient
    {
        ...
        public List<(ReadOnlyMemory<char> Message, WebSocketMessageType Type)> TextSendInstantCalls { get; } = [];
        public List<(ReadOnlyMemory<byte> Message, WebSocketMessageType Type)> BinarySendInstantCalls { get; } = [];
        ...
        public Func<ReadOnlyMemory<char>, WebSocketMessageType, Task<bool>> SendInstantTextHandler { get; set; }
            = (_, _) => Task.FromResult(true);

        public Func<ReadOnlyMemory<byte>, WebSocketMessageType, Task<bool>> SendInstantBinaryHandler { get; set; }
            = (_, _) => Task.FromResult(true);
        ...
        {
            TextSendInstantCalls.Add((message, type));
            return SendInstantTextHandler(message, type);
        }
        public Task<bool> SendInstantAsync(ReadOnlyMemory<byte> message, WebSocketMessageType type,
            CancellationToken cancellationToken = default)
        {
            BinarySendInstantCalls.Add((message, type));
            return SendInstantBinaryHandler(message, type);
        }
    ...
    private sealed class TestReactiveWebSocketServer : IReactiveWebSocketServer
    {
        ...
        public List<(Guid ClientId, ReadOnlyMemory<char> Message, WebSocketMessageType Type)> TextSendInstantCalls { get; } = [];
        public List<(Guid ClientId, ReadOnlyMemory<byte> Message, WebSocketMessageType Type)> BinarySendInstantCalls { get; } = [];
        ...
        public Func<Guid, ReadOnlyMemory<char>, WebSocketMessageType, Task<bool>> SendInstantTextHandler { get; set; }
            = (_, _, _) => Task.FromResult(true);

        public Func<Guid, ReadOnlyMemory<byte>, WebSocketMessageType, Task<bool>> SendInstantBinaryHandler { get; set; }
            = (_, _, _) => Task.FromResult(true);
        ...
        public Task<bool> SendInstantAsync(Guid clientId, ReadOnlyMemory<char> message, WebSocketMessageType type,
            CancellationToken cancellationToken = default)
        {
            TextSendInstantCalls.Add((clientId, message, type));
            return SendInstantTextHandler(clientId, message, type);
        }
        public Task<bool> SendInstantAsync(Guid clientId, ReadOnlyMemory<byte> message, WebSocketMessageType type,
            CancellationToken cancellationToken = default)
        {
            BinarySendInstantCalls.Add((clientId, message, type));
            return SendInstantBinaryHandler(clientId, message, type);
        }
